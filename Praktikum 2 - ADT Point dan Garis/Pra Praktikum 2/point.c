...
#include "point.h" // " artinya dr file external. <> artinya dr library compiler
#include "boolean.h"
#include <assert.h>
#include <stdio.h>
#include <math.h>
...
{
    POINT P;
    GetAbsis(P) += GetAbsis(p) + deltaX;
    GetOrdinat(P) += GetOrdinat(p) + deltaY;
    return P;
}
...
{
    sqrt((pow(GetAbsis(p1) - GetAbsis(p2), 2)) + (pow(GetOrdinat(p1) - (GetOrdinat(p2)), 2)));
}
