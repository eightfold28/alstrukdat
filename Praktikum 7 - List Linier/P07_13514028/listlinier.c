 ...
 * FS. L1 dan L2 kosong, L3 adalah hasil konkatenasi L1 dan L2.
 * Konkatenasi L1 dan L2 menghasilkan L3 yang baru dengan elemen list L1 dan L2, dan 
 * L1 dan L2 menjadi list kosong. Tidak ada alokasi/ dealokasi
*/
{
	Address P;
	
	Concat(*L1, *L2, L3);
    while (IsListEmpty(*L1) == false) {
        DeleteFirst(L1, &P);
	}
	while (IsListEmpty(*L2) == false) {
        DeleteFirst(L2, &P);
	}
